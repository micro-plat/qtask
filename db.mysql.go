...
const sqlDelayTask = `update tsk_system_task t set t.next_execute_time= date_add(now(),interval t.execute_interval second)
where t.status in(20,30) and t.task_id=@task_id`

const sqlProcessingTask = `update tsk_system_task t set t.next_execute_time=date_add(now(),interval t.execute_interval second),
...
const sqlUpdateTask = `update tsk_system_task t set t.batch_id=@batch_id,t.next_execute_time= date_add(now(),interval t.execute_interval second)
